<?php
	...
	if(isset($_POST['name']) && isset($_POST['email']) && isset($_POST['password'])  && isset($_POST['mobile']) && isset($_POST['gender']) && isset($_POST['date'])  && isset($_POST['month']) && isset($_POST['date']) && isset($_POST['year'])) {
		...
		if(!empty($name) && !empty($email) && !empty($password) && !empty($mobile) && !empty($gender) && !empty($date) && !empty($month) && !empty($year)) {
			...
			echo '<div class="alert alert-warning"><a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a><strong>Fields Left Blank. Please re-check your feeds.</strong><br></div>';
		}	
	}
...
<button type="button" onclick="location.href='index.php';" class="btn btn-block btn-success">Log in</button>
</div></h2>
</div>
